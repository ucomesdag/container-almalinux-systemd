AlmaLinux Systemd Container Image
=====================

This Containerfile can build containers capable to use systemd.

[![almalinux container image][quai.io]](https://quay.io/repository/ucomesdag/almalinux?tab=tags)

Branches
--------

This repository has multiple branches that relate to CentOS versions.

|Branch |AlmaLinux Version |Container image tag|
|-------|------------------|-------------------|
|main   |latest (9)        |latest             |
|8      |8                 |8                  |

Pull strategy
-------------

The different branches are **not** merged, they live as individual branches.

Manually starting
-----------------

```
podman run \
  --tty \
  --privileged \
  --volume /sys/fs/cgroup:/sys/fs/cgroup:ro \
  quay.io/ucomesdag/almalinux:latest
```

<!-- container image -->

[quai.io]: https://img.shields.io/badge/Quay.io-container%20image-green?style=flat&logo=data:image/png;base64,&logoColor=white
